...
await (async () => {
...

  {
    ...
    let patchRequestBody: Record<string, unknown> | undefined;
    ...
    setFetch(async (input, init) => {
      ...
      if (path === '/api/assessments' && init?.method === 'POST') {
        createRequestBody = JSON.parse(String(init.body ?? '{}')) as Record<
          string,
          unknown
        >;
        ...
        return createJsonResponse({
          data: {
            id: 'asm_2',
            companyId: 'cmp_1',
            title: String(createRequestBody.title ?? ''),
            description: String(createRequestBody.description ?? ''),
            scope: String(createRequestBody.scope ?? ''),
            status: 'draft',
            startedAt: undefined,
            completedAt: undefined,
            applicationName: String(createRequestBody.applicationName ?? ''),
            environment: undefined,
            assessmentType: String(createRequestBody.assessmentType ?? ''),
      ...
      if (path === '/api/assessments/asm_1' && init?.method === 'PATCH') {
        patchRequestBody = JSON.parse(String(init.body ?? '{}')) as Record<
          string,
          unknown
        >;
        ...
        return createJsonResponse({
          data: {
            id: 'asm_1',
            companyId: 'cmp_1',
            title: String(patchRequestBody.title ?? ''),
            description: String(patchRequestBody.description ?? ''),
            scope: String(patchRequestBody.scope ?? ''),
            status: String(patchRequestBody.status ?? ''),
            startedAt: undefined,
            completedAt: undefined,
            applicationName: String(patchRequestBody.applicationName ?? ''),
            environment: undefined,
            assessmentType: String(patchRequestBody.assessmentType ?? ''),
            overallRisk: undefined,
            owaspTaxonomyVersion: OWASP_TOP_10_CURRENT_VERSION,
    ...
    try {
      ...
      const newAssessmentButton = Array.from(
        container.querySelectorAll('button'),
      ).find(button => button.textContent?.includes('New assessment')) as
        | HTMLButtonElement
        | undefined;

      assert.ok(newAssessmentButton, 'Expected the create assessment button');
      ...
      const submitButton = Array.from(
        container.querySelectorAll('button'),
      ).find(button => button.textContent?.includes('Create assessment')) as
        | HTMLButtonElement
        | undefined;

      assert.ok(applicationInput, 'Expected the application field in create');
      assert.ok(nameInput, 'Expected the name field in create');
      assert.ok(submitButton, 'Expected the create submit button');

      await act(async () => {
        applicationInput!.value = 'Customer Services Portal';
      ...
      assert.equal(createRequestBody, undefined);
      assert.ok(
        textContent(container).includes('Assessment name is required.'),
        'Expected a local validation message for the missing assessment name',
      );
      assert.equal(
        applicationInput!.value,
        'Customer Services Portal',
        'Expected the entered application value to remain after validation failure',
      );

      await act(async () => {
        nameInput!.value = 'Customer Services Portal Assessment';
        nameInput!.dispatchEvent(
          new window.Event('input', {
            bubbles: true,
            cancelable: true,
      ...
      assert.equal(createRequestBody?.companyId, 'cmp_1');
      assert.equal(
        createRequestBody?.applicationName,
        'Customer Services Portal',
      );
      assert.equal(
        createRequestBody?.title,
        'Customer Services Portal Assessment',
      );

      const editButton = Array.from(
        container.querySelectorAll('.assessment-table__row button'),
      ).find(button => button.textContent?.includes('Edit')) as
        | HTMLButtonElement
        | undefined;

      assert.ok(editButton, 'Expected the edit action');
      ...
      const editApplicationInput = container.querySelector(
        '#assessment-application-name',
      ) as HTMLInputElement | null;
      const editSubmitButton = Array.from(
        container.querySelectorAll('button'),
      ).find(button => button.textContent?.includes('Save changes')) as
        | HTMLButtonElement
        | undefined;

      assert.ok(editApplicationInput, 'Expected the application field in edit');
      assert.equal(
        editApplicationInput!.value,
        'Customer Services Portal',
        'Expected edit form to load the existing application value',
      );

      await act(async () => {
        editApplicationInput!.value = 'Customer Services Portal Public Site';
        editApplicationInput!.dispatchEvent(
          new window.Event('input', {
            bubbles: true,
            cancelable: true,
          }),
      ...
      assert.equal(patchRequestBody?.companyId, undefined);
      assert.equal(
        patchRequestBody?.applicationName,
